...
export function PollVoteForm({
  ...
  const canSubmitVote = canAny([...VOTING_PERMISSION_KEYS.VOTE.CREATE]);
  ...
  }, [pollId, permissionsLoaded, permissionsLoading, canSubmitVote]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (msg === "NOT_ELIGIBLE")
        setError("You are not eligible to vote in this poll.");
      else if (msg === "ALREADY_VOTED" || /already.*vot/i.test(msg))
  ...
  const isNotEligibleError =
    error != null && /not eligible|NOT_ELIGIBLE|eligibility/i.test(error);

  const handleAddMeAsEligible = async () => {
    if (!canCreateEligibility) return;
  ...
  if (!permissionsLoaded || permissionsLoading) {
    return (
      <div className={`text-center py-3 ${className}`}>
        ...
        <p className="text-muted small mb-0 mt-2">Checking permissions...</p>
      </div>
    );
  }
  ...
  if (!canSubmitVote) {
    return (
      <div className={`alert alert-warning py-3 ${className}`} role="alert">
        <i className="bi bi-shield-lock me-2"></i>
        You do not currently have permission to submit a vote for this poll.
      </div>
    );
  }
  ...
  return (
    <div
      className={`poll-vote-form ${embedded ? "poll-vote-form--embedded" : ""} ${className}`}
    >
      <h3 className={`fw-semibold ${embedded ? "h6 mb-2" : "mb-2"}`}>
        {poll
          ? asText((poll as unknown as Record<string, unknown>).title)
          : "Vote"}
      </h3>
            ...
            canCreateEligibility && (
              <div className="mt-2 pt-2 border-top border-danger">
                <p className="mb-2 small">
                  You created this poll. Add yourself as eligible to vote:
                </p>
                <button
                  type="button"
                ...
                <Link
                  to={`/dashboard/voting/${pollId}`}
                  state={{ tab: "eligibility" }}
      ...
      {options.length === 0 ? (
        ...
        <div
          className={`card border-0 shadow-sm rounded-3 overflow-hidden ${embedded ? "" : ""}`}
        >
                    ...
                    <span className={isSelected ? "fw-medium" : ""}>
                      {asText(
                        (opt as unknown as Record<string, unknown>)
                          .optionText ??
                          (opt as unknown as Record<string, unknown>).title,
                      )}
                    </span>
                  </label>
                );
