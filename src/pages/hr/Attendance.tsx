...
export default function Attendance() {
    ...
    const fetchStats = async () => {
        try {
            const res = await hrService.getAttendanceStats(selectedEmployee, month, year);
    ...
    const handleMarkAbsence = async () => {
        ...
        try {
            await hrService.markAbsence({ employeeId: selectedEmployee, date: new Date().toISOString().split('T')[0], reason: reason || undefined });
            alert('Absence recorded!');
            fetchRecords();
        } catch (err: any) { alert(err.response?.data?.message || 'Failed to mark absence'); }
    };
        ...
        <div className="attendance-container">
            <div className="attendance-header">
                <div className="header-content">
                    <h1 className="page-title"><i className="bi bi-clock-history me-3"></i>Attendance</h1>
                    <p className="page-subtitle">Track employee check-ins, check-outs, and attendance records</p>
                </div>
            </div>
            ...
            <div className="attendance-controls">
                ...
                <div className="control-group">
                    <label>Month</label>
                    <select className="form-select" style={{ borderRadius: 8, width: 120 }} value={month} onChange={e => setMonth(+e.target.value)}>
                        {Array.from({ length: 12 }, (_, i) => <option key={i + 1} value={i + 1}>{new Date(2024, i).toLocaleString('default', { month: 'short' })}</option>)}
                    </select>
                </div>
                <div className="control-group">
                    <label>Year</label>
                ...
                <>
                    ...
                    {stats && (
                        <div className="stats-grid">
                            <div className="stat-card"><div className="stat-value">{stats.totalDays ?? '—'}</div><div className="stat-label">Total Days</div></div>
                            <div className="stat-card"><div className="stat-value">{stats.presentDays ?? '—'}</div><div className="stat-label">Present</div></div>
                            <div className="stat-card"><div className="stat-value">{stats.absentDays ?? '—'}</div><div className="stat-label">Absent</div></div>
                            <div className="stat-card"><div className="stat-value">{stats.totalHours ?? '—'}</div><div className="stat-label">Total Hours</div></div>
                        </div>
                    )}
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td><span className={`status-badge status-${r.status || 'ACTIVE'}`}>{r.status || 'Present'}</span></td>
                                    </tr>
                                )) : (
                                    <tr><td colSpan={5} style={{ textAlign: 'center', padding: '3rem', color: '#95a5a6' }}>No attendance records found</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
